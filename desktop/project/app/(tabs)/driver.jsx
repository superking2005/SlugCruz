...
export default function DriverRidesScreen() {
  ...
  const fetchRides = async () => {
    ...
    if (error) {
      ...
      console.log('Fetched rides with signups:', JSON.stringify(data, null, 2));
      setRides(data);
    }
  };
  ...
  const fetchRiderScheduledRides = async () => {
    ...
    if (error) {
      ...
      console.log('Fetched rider scheduled rides:', JSON.stringify(data, null, 2));
  ...
  const bookRiderScheduledRide = async (rideId) => {
    ...
    if (error) {
      ...
      fetchRiderScheduledRides();
      // Refresh filtered results if filters are currently applied
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <View style={styles.rideCard}>
                  ...
                  {/* Show profile info if it's a rider-posted ride */}
                  {item.posted_by === 'rider' && (
                    <>
                      <Text style={styles.riderName}>
                        Rider: {item.profiles?.full_name || 'No name available'}
                      </Text>
                      <Text>Major: {item.profiles?.major || 'No major listed'}</Text>
                      <Text>College: {item.profiles?.college || 'No college listed'}</Text>
                  ...
                  {/* Show ride signups if it's a driver-posted ride */}
                  {item.posted_by === 'driver' && item.ride_signups?.map((signup) => (
                    <View key={signup.id} style={styles.signupCard}>
                      <Text style={styles.riderName}>Rider: {signup.profiles?.full_name || 'N/A'}</Text>
                      <Text>Major: {signup.profiles?.major || 'N/A'}</Text>
                      <Text>College: {signup.profiles?.college || 'N/A'}</Text>
          ...
          renderItem={({ item }) => (
            <View style={styles.rideCard}>
              ...
              {item.ride_signups?.map((signup) => {
                ...
                return (
                  <View key={signup.id} style={styles.signupCard}>
                    <Text style={styles.riderName}>Rider: {signup.profiles?.full_name || 'No name available'}</Text>
                    <Text>Major: {signup.profiles?.major || 'No major listed'}</Text>
                    <Text>College: {signup.profiles?.college || 'No college listed'}</Text>
                    <Text>Status: {signup.booked}</Text>
                    <Text style={{fontSize: 12, color: '#666', marginTop: 5}}>
                      Debug: Rider ID - {signup.rider_id}
                    </Text>
                    {signup.booked === 'pending' && (
                      <View style={styles.buttonRow}>
                        <Button title="Accept" onPress={() => updateBookingStatus(signup.id, 'accepted')} />
